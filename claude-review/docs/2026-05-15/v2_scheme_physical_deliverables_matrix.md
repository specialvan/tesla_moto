...
```text
engineering/v2/S<scheme_id>_<scheme_slug>/
  pcb/
  cad/
  controller/
  bom_eda/
...
| 标签 | 含义 | 可进入阶段 |
|---|---|---|
| `missing` | 尚无图纸或只有文字描述 | 不能过 G2 |
| `stub` | 有占位图纸/接口草案 | G0-G1 |
| `draft` | 有可评审初稿 | G2-G3 |
...
|---|---|---|---|---|---|
| PCB | 电流采样、Vdc 采样、相电压重建、温度采样、gate-disable / fault latch 页 | `V2-S01-PCB-sensing_fault_path-r00.pdf` | G4 | ERC/DRC 通过，采样精度与 fault latch 路径可追溯 | `stub` |
...
| 类别 | 必需交付物 | 命名模板 | Gate | 验收条件 | 当前状态 |
|---|---|---|---|---|---|
| PCB | MCU/NVM、resolver、ADC、电流采样、CAN/诊断接口页 | `V2-S02-PCB-control_io_map-r00.pdf` | G4 | 控制 LUT 存储、刷新、CRC、诊断链路明确 | `stub` |
...
| Controller | SVPWM/六步/过调制状态机、进入/退出窗口、THD/NVH 限制 | `V2-S03-CTRL-overmodulation_state_machine-r00.drawio` | G3 | 退出条件、故障回退和调制因子限制可测试 | `stub` |
...
| 类别 | 必需交付物 | 命名模板 | Gate | 验收条件 | 当前状态 |
|---|---|---|---|---|---|
| PCB | 无新增功率页；需要温度、位置、电流采样精度证明页 | `V2-S04-PCB-lut_observer_inputs-r00.pdf` | G4 | LUT 输入信号精度支持插值误差预算 | `stub` |
...
| Controller | 饱和 LUT 回灌、候选几何 scorecard、控制轨迹对比图 | `V2-S05-CTRL-saturation_scorecard-r00.drawio` | G3 | FEA LUT 回灌后收益仍成立 | `stub` |
...
| Controller | 高凸极 MTPA/MTPV 控制图、低 PM 退磁边界图 | `V2-S06-CTRL-saliency_control_map-r00.drawio` | G3 | 主工况效率和峰值转矩不被牺牲 | `stub` |
...
| Controller | 磁状态机、状态观测、未知状态回退、重复切换寿命计数 | `V2-S07-CTRL-flux_state_machine-r00.drawio` | G3 | 不能用 `psi_f` 缩放代替真实磁状态 | `stub` |
...
| Controller | `id/iq/if` 三变量控制图、loss-of-field fallback | `V2-S08-CTRL-three_variable_control-r00.drawio` | G3 | 励磁损耗和故障进入可行性判断 | `stub` |
...
| Controller | 切换状态机、零转矩窗口、非法状态回退、环流检测 | `V2-S09-CTRL-winding_reconfig_state_machine-r00.drawio` | G3 | 不能只用静态 Ke/Kt 缩放 | `stub` |
...
| Controller | fault torque allocator、谐波子空间、相组降额状态机 | `V2-S10-CTRL-fault_torque_allocator-r00.drawio` | G3 | 不能只用可用电流降额代理 | `stub` |
...
| 类别 | 必需交付物 | 命名模板 | Gate | 验收条件 | 当前状态 |
|---|---|---|---|---|---|
| PCB | 磁钢/油温/绕组温度采样、gate-disable、fault latch、ASIL 诊断页 | `V2-S11-PCB-safety_fault_latch-r00.pdf` | G4 | 故障检测、锁存、关断链路和诊断覆盖明确 | `stub` |
...
| 类别 | 必需交付物 | 命名模板 | Gate | 验收条件 | 当前状态 |
|---|---|---|---|---|---|
| PCB | 不新增硬件；必须索引候选方案 PCB/BOM/EDA 版本 | `V2-S12-PCB-candidate_index-r00.md` | G4 | Pareto 输入引用真实硬件版本 | `stub` |
| 3D/CAD | 候选拓扑 CAD 版本索引、质量、封装、冷却边界 | `V2-S12-CAD-candidate_package_index-r00.md` | G3 | 每个候选有可比较 CAD/质量/尺寸来源 | `stub` |
| Controller | 决策 scorecard、drive cycle、系统损耗、成熟度权重图 | `V2-S12-CTRL-pareto_scorecard-r00.drawio` | G3 | 区分 research rank 与 engineering recommendation | `draft` |
