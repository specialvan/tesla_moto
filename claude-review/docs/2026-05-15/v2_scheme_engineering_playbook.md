...
本 playbook 定义每条路线的推进策略、模型输入、控制接口和阶段门条件；具体 PCB、3D/CAD、控制器图、BOM/EDA、DVP&R/DFMEA 的可见图纸包要求，以 `v2_scheme_physical_deliverables_matrix.md` 为准。

V2 后续不得只用本 playbook 的文字描述宣称工程落地。任何方案进入 G2 之后，都必须能在物理交付物矩阵中定位对应图纸包、命名模板、当前状态和 gate blocker。
