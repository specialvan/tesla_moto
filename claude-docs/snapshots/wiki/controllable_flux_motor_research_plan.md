...
当前同步口径（2026-05-29）：EXP-001 到 EXP-011 属于可重跑数值/代理实验范围；EXP-011 是 Bertotti 三项简化铁耗 proxy 扫描，`freq_out_of_range_points` 用于标出 400 Hz 系数有效域外的高速外推风险。r02 sim_binding 与 r03 production drawing pack 仅作为机器可读仿真锚点和图档提示词证据，统一保持 `engineering_validated=false`，不代表 FEA、HIL、台架或量产释放结论。
...
EXP-010 / EXP-011 同步结论：EXP-010 已提供示意工况下的加权效率 Pareto 路由筛选，但评分仍依赖 proxy 权重和简化损耗项；EXP-011 已提供 Bertotti 三项简化铁耗扫描，`freq_out_of_range_points=48` 暴露高速频率超出 400 Hz 系数有效域的外推风险。二者都只能支撑方案筛选和风险排序，下一步必须补材料片参数、FEA 磁密分布、PWM 谐波、热耦合、HIL 和台架证据。
...
| EXP-010 | 加权效率 Pareto | 示意工况权重、候选路线、简化损耗项 | 路线排序、可行性、敏感性 | proxy 排序可复现，不代表客户工况闭环 |
| EXP-011 | Bertotti 铁耗扫描 | 线性 dq 点、Bertotti 系数、频率上限 | 铁耗趋势、效率、`freq_out_of_range_points` | 暴露高速铁耗风险，不代表材料/FEA/HIL/台架验证 |
